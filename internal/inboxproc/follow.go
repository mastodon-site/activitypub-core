...
func handleFollow(ctx context.Context, pool *pgxpool.Pool, q queue.Backend, cfg *config.Config, httpClient *http.Client, fetchPolicy *fetch.Policy, row *store.ActivityRow, fields map[string]json.RawMessage) error {
	...
	inbox, err := fetch.InboxURLFromReference(ctx, httpClient, fetchPolicy, followerActorURL, cfg)
	if err != nil {
...
func handleAccept(ctx context.Context, pool *pgxpool.Pool, row *store.ActivityRow, fields map[string]json.RawMessage) error {
	...
	err = pool.QueryRow(ctx, `
		SELECT follower_actor_id, followee_actor_id FROM follows WHERE follow_activity_id = $1
	`, target).Scan(&followerID, &followeeID)
	if err != nil {
	...
	if followeeID != row.ActorID {
		return fmt.Errorf("accept: activity actor is not the followee for this follow")
	}
	return store.SetFollowStateByFollowActivityIDForFollower(ctx, pool, target, store.FollowStateAccepted, followerID)
}
...
func handleReject(ctx context.Context, pool *pgxpool.Pool, row *store.ActivityRow, fields map[string]json.RawMessage) error {
	...
	if followeeID != row.ActorID {
		return fmt.Errorf("reject: activity actor is not the followee for this follow")
	}
	return store.SetFollowStateByFollowActivityIDForFollower(ctx, pool, target, store.FollowStateRejected, followerID)
}
